//
//  SSHomeAddTestSubjectContentCell.m
//  BONEDE
//
...
@interface SSHomeAddTestSubjectContentCell ()
...
@implementation SSHomeAddTestSubjectContentCell
...
- (IBAction)nextAction:(UIButton *)sender {
    if(!kMeUnNilStr(_model.content).length){
        [SSShowViewTool showMessage:@"请先填写题目" view:self];
        return;
    }
    if(!kMeUnNilStr(_model.option1).length){
        [SSShowViewTool showMessage:@"请先填写问题一" view:self];
        return;
    }
    if(!kMeUnNilStr(_model.option2).length){
        [SSShowViewTool showMessage:@"请先填写问题二" view:self];
        return;
    }
    if(!kMeUnNilStr(_model.option3).length){
        [SSShowViewTool showMessage:@"请先填写问题三" view:self];
        return;
    }
    if(!kMeUnNilStr(_model.option4).length){
        [SSShowViewTool showMessage:@"请先填写问题四" view:self];
        return;
    }
